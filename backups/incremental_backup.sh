...
FULL_BACKUP_LIMIT=4
BACKUPS_TO_KEEP=12
EXCLUSSIONS="--exclude .cache/ --exclude .thumbnails/ --exclude .gvfs --exclude files_trashbin/ "
OPTIONS="-h -ab --stats"
#To test the script, include "-n" to perform a 'dry' rsync
LOG=/var/log/backup-$DATE-$HHMMSS.log
...
if [ $? -ne "0" ]; then
    ...
    echo "[" `date +%Y-%m-%d_%R` "]" "Error during the backup" >> $LOG
    echo "" >> $LOG
    echo "" >> $LOG
else
